...
_TD.a.push(function (TD) {
	...
	var panel_obj = {
		...
		render: function () {
			...
			ctx.fillStyle = "#000";
			ctx.font = "normal " + (12 * _TD.retina) + "px 'Helvetica'";
			ctx.beginPath();
			...
			if (this._life_recover_wait) {
				...
				ctx.font = "bold " + (12 * _TD.retina) + "px 'Helvetica'";
				ctx.beginPath();
			...
			ctx.font = "normal " + (12 * _TD.retina) + "px 'Helvetica'";
			ctx.beginPath();
			ctx.fillText("FPS: " + TD.fps + " | CookieGames", TD.padding, TD.stage.height - TD.padding * 2);
			ctx.closePath();
		}
	};
	...
	var balloontip_obj = {
		...
		msg: function (txt, el) {
			this.text = txt;
			var ctx = TD.ctx;
			ctx.font = "normal " + (12 * _TD.retina) + "px 'Arial'";
			this.width = Math.max(
	...
	var button_obj = {
		_init: function (cfg) {
			...
			this.x = cfg.x;
			this.y = cfg.y;
			this.width = cfg.width || 100 * _TD.retina;
		...
		render: function () {
			...
			ctx.fillStyle = this.is_hover ? "#eee" : "#fff";
			...
			ctx.font = "normal " + (12 * _TD.retina) + "px 'Arial'";
			ctx.beginPath();
			ctx.fillText(this.text, this.font_x + 13, this.font_y);
			ctx.font = "12px FontAwesome";
			//hacky - TODO: find out why using a variable here causes an undefined icon
			if(this.text == "Pause"){
				ctx.fillText("\uf04c", this.font_x, this.font_y);
			}
